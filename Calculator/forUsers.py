import easygui
"""This folder is for function which is user-friendly. Users just put their data_pre in the function."""


def vel(data, cols=None):
    """
    this function calculates velocity at each frame from AVATAR csv file
    :param data: raw data_pre, csv file from AVATAR
    ...
    if cols is None:    # If users enter no specific columns name, multichoice box will pop up.
        msg = "Which data_pre columns you want to use for calculating velocity ?"
...
def accel(data, cols=None):
    ...
    if cols is None:    # If users enter no specific columns name, multichoice box will pop up.
        msg = "Which data_pre columns you want to use for calculating acceleration ?"
